...
import { hexToPixel, MAP_ROTATION, rotatePoint } from "../hexMath";
...
/**
 * Render static tile outlines for tiles that will be animated in.
 * These outlines fade in with the board before the tracer animation starts.
 * Returns a container with all the outlines that can be removed later.
 */
export function renderStaticTileOutlines(
  container: Container,
  ...
  outlinesContainer.label = "static-tile-outlines";

  // Get the cluster vertices (same as TileOutlineTracer uses)
  const rawVertices = get7HexClusterVertices(HEX_SIZE);
  const vertices = rawVertices.map(v => rotatePoint(v.x, v.y));

  if (vertices.length === 0) {
    container.addChild(outlinesContainer);
    return outlinesContainer;
  }
  ...
  for (const tile of tiles) {
    ...
    outlinesContainer.addChild(graphics);
  }
  ...
  setTimeout(() => {
    ...
    animManager.animate(`tile-drop-${tileId}`, sprite, {
      ...
      onComplete: () => {
        ...
        // Apply tile effects now that tile is visible
 ...
 *
 * Similar to intro animation with callback for sequencing enemies after tile lands.
 */
...
export async function renderTiles(
  ...
  for (const tile of tiles) {
    ...
    try {
      // Texture is already cached from parallel preload above
      const texture = Assets.get<Texture>(imageUrl);
      ...
      if (playIntro && animManager && particleManager) {
        // During intro, DON'T apply tile effects yet - they would be visible
        // before the tile drops. Effects will be applied when tile lands.
        sprite.alpha = 0;
        ...
        // Exploration reveal - don't apply effects yet, apply when tile lands
        sprite.alpha = 0;
        sprite.scale.set(targetScaleX * 0.8, targetScaleY * 0.8);
        ...
        // Not intro/reveal - apply effects immediately (tile already visible)
        applyTileEffects(sprite, layers.shadows, position, HEX_SIZE, {
          enableShadow: true,
          enableBorder: true,
          enableWarmTint: true,
