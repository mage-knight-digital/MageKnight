...
import { SKILL_WOLFHAWK_REFRESHING_BATH } from "../../data/skills/index.js";
...
describe("Refreshing Bath skill (Wolfhawk)", () => {
  ...
  describe("activation", () => {
    it("should grant Heal 1 and 1 blue crystal", () => {
      const player = createTestPlayer({
        hero: Hero.Wolfhawk,
        skills: [SKILL_WOLFHAWK_REFRESHING_BATH],
      ...
      const result = engine.processAction(state, "player1", {
        ...
        skillId: SKILL_WOLFHAWK_REFRESHING_BATH,
      });
    ...
    it("should add skill to usedThisRound cooldown (once per round)", () => {
      ...
      ).toContain(SKILL_WOLFHAWK_REFRESHING_BATH);
    });
  ...
  describe("crystal limit", () => {
    it("should still heal when at max blue crystals", () => {
      const player = createTestPlayer({
        hero: Hero.Wolfhawk,
        skills: [SKILL_WOLFHAWK_REFRESHING_BATH],
        skillCooldowns: buildSkillCooldowns(),
  ...
  describe("valid actions", () => {
    ...
    it("should not show skill when on round cooldown", () => {
      const player = createTestPlayer({
        ...
        skillCooldowns: {
          ...buildSkillCooldowns(),
          usedThisRound: [SKILL_WOLFHAWK_REFRESHING_BATH],
        },
        hand: [CARD_MARCH],
      });
      const state = createTestGameState({ players: [player] });
  ...
  describe("undo", () => {
    it("should restore cooldown and crystal on undo", () => {
      ...
      ).not.toContain(SKILL_WOLFHAWK_REFRESHING_BATH);
