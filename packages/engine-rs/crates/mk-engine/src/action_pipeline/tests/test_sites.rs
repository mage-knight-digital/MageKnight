...
fn buy_spell_multiple_offers_enumerated() {
    ...
    // Give player red + blue + green mana to cover all 3 spells
    state.players[0].pure_mana.push(ManaToken { color: ManaColor::Red, source: ManaTokenSource::Effect, cannot_power_spells: false });
    state.players[0].pure_mana.push(ManaToken { color: ManaColor::Blue, source: ManaTokenSource::Effect, cannot_power_spells: false });
    state.players[0].pure_mana.push(ManaToken { color: ManaColor::Green, source: ManaTokenSource::Effect, cannot_power_spells: false });
...
#[test]
fn unit_reward_queued_as_deferred() {
    let mut state = setup_playing_game(vec!["march"]);
    state.offers.units = vec![mk_types::ids::UnitId::from("peasants")];

    queue_site_reward(&mut state, 0, SiteReward::Unit);
    ...
    assert!(
        ...
        "Unit reward should be promoted to active pending for player choice"
    );
}

#[test]
fn unit_reward_skipped_when_no_units_available() {
    let mut state = setup_playing_game(vec!["march"]);
    ...
    assert!(
        ...
        "Unit reward should be auto-skipped when no units in offer"
    );
}

#[test]
fn unit_reward_select_adds_unit_to_player() {
    let mut state = setup_playing_game(vec!["march"]);
    state.offers.units = vec![
        mk_types::ids::UnitId::from("peasants"),
        mk_types::ids::UnitId::from("foresters"),
    ];
    state.decks.unit_deck = vec![mk_types::ids::UnitId::from("herbalists")];
    state.players[0].pending.active = Some(ActivePending::SiteRewardChoice {
        reward: SiteReward::Unit,
        reward_index: 0,
    });
    ...
    apply_legal_action(
        ...
        &LegalAction::SelectReward {
            card_id: CardId::from(""),
            reward_index: 0,
            unit_id: Some(mk_types::ids::UnitId::from("peasants")),
        },
        epoch,
    ).unwrap();
    ...
    assert_eq!(state.players[0].units[0].unit_id.as_str(), "peasants");
    assert_eq!(state.players[0].units[0].state, UnitState::Ready);

    // Unit removed from offer and replenished
    assert!(!state.offers.units.iter().any(|u| u.as_str() == "peasants"));
    assert!(state.offers.units.iter().any(|u| u.as_str() == "herbalists"));
    assert_eq!(state.offers.units.len(), 2); // foresters + herbalists
}

#[test]
fn unit_reward_enumerated_for_unit_offer() {
    let mut state = setup_playing_game(vec!["march"]);
    state.offers.units = vec![
        mk_types::ids::UnitId::from("peasants"),
        mk_types::ids::UnitId::from("foresters"),
    ...
    let actions = enumerate_legal_actions_with_undo(&state, 0, &UndoStack::new());
    let rewards: Vec<_> = actions.actions.iter()
        .filter(|a| matches!(a, LegalAction::SelectReward { .. }))
        .collect();
    assert_eq!(rewards.len(), 2, "Should enumerate one SelectReward per unit in offer");

    // Verify unit_ids are set
    for reward in &rewards {
        if let LegalAction::SelectReward { unit_id, .. } = reward {
            assert!(unit_id.is_some(), "Unit rewards should have unit_id set");
        }
    }
}

#[test]
fn ruins_token_unit_reward_end_to_end() {
    // enemy_green_violet_artifact → Unit reward
    let mut state = setup_playing_game(vec!["march"]);
    let mut undo = UndoStack::new();
    state.offers.units = vec![
        mk_types::ids::UnitId::from("peasants"),
    ...
    setup_ruins_combat_with_token(
        ...
        vec![("green", "orc_summoners_1"), ("violet", "monks_1")],
    );
    win_ruins_combat(&mut state, &mut undo);

    // Unit reward should be promoted to active pending
    assert!(
        matches!(
            state.players[0].pending.active,
            Some(ActivePending::SiteRewardChoice {
        ...
        "Unit reward should create SiteRewardChoice pending"
    );
...
fn shield_bonus_once_per_turn() {
    ...
    // Flag should be set
    assert!(state.players[0].flags.contains(PlayerFlags::SHIELD_INFLUENCE_CLAIMED_THIS_TURN));
