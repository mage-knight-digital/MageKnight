...
locals {
  ...
  asset_pack_path           = "mageknight/v1/assets"
  assets_base_url           = "https://assets.${local.domain_name}/${local.asset_pack_path}"
...
module "cloudflare_site" {
  ...
  assets_bucket_name = "mageknight-assets-prod"
...
module "hetzner_app" {
  ...
  server_name         = "mage-knight-digital-prod"
  environment         = "prod"
